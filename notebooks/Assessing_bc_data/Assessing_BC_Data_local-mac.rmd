...
input <- "raster" #Either df or raster -- R outputs are a group of dfs in list form saved as an RDS, python input is a raster
city <- "Glasgow" 
var <- "tasmax" 
runs <- c("05", "06", "07", "08")
...
####### PYTHON INPUTS HERE ######
  ...
  #dfs are assumed to be cells x time 
   r <- list.files(paste0(dd, "Debiased/three.cities.cropped/Manchester/08/tasmax/"), full.names = T)[1]
  rast <- rast(r)
  crs <- crs(rast)
