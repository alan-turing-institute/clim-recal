...
dd <- "/mnt/vmfileshare/ClimateData/" #Data directoy of all data used in this script
...
####### PYTHON INPUTS HERE ######
  ...
  ## Load a source raster to extract the crs
   ## Also to add needs to be changing the names of the dfs to make then same acorss all methods 
  ...
  obs.val.df.L <- lapply(allruns, function(L){t(L[["t.obs"]])}) #This object stored in the results transposed for applying bias correction - maybe look at adding this to the bias correction script rather than here? 
  obs.cal.df.L <- lapply(allruns, function(L){L[["val.df"]]})
  
  cpm.cal.raw.df.L <- lapply(allruns, function(L){t(L[["t.cal"]])}) 
...
#here - sorting out inputs and rasters in single chunk
```{r}
