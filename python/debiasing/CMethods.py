...
class CMethods_climrecal():
    ...
    def linear_scaling(cls,
        ...
        '''
        if group is not None: return cls.grouped_correction(method='linear_scaling', obs=obs, simh=simh, simp=simp,
                                                            group=group, kind=kind, **kwargs)
    ...
    def variance_scaling(cls,
        ...
        ----- R E F E R E N C E S -----
            ...
            https://doi.org/10.1016/j.jhydrol.2012.05.052
        '''
        if group is not None: return cls.grouped_correction(method='variance_scaling', obs=obs, simh=simh, simp=simp,
                                                            group=group, kind=kind, **kwargs)
    ...
    def delta_method(cls,
        ...
        if group is not None: return cls.grouped_correction(method='delta_method', obs=obs, simh=simh, simp=simp,
                                                            group=group, kind=kind, **kwargs)
    ...
    def quantile_mapping(cls,
        ...
        if group is not None: return cls.grouped_correction(method='quantile_mapping', obs=obs, simh=simh, simp=simp,
                                                            group=group, n_quantiles=n_quantiles, kind=kind, **kwargs)
    ...
    def quantile_delta_mapping(cls,
        ...
        if group is not None: return cls.grouped_correction(method='quantile_delta_mapping', obs=obs, simh=simh,
                                                            simp=simp, group=group, n_quantiles=n_quantiles, kind=kind,
                                                            **kwargs)
        if kind in cls.ADDITIVE:
            res = simp.copy(deep=True)
