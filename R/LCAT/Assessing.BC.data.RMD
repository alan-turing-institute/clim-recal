...
obs.daymeans.df$season_year <- ifelse(grepl("0131_|0228_|0229_", x), 
                                      paste0(obs.daymeans.df$season, year-1), 
                                      paste0(obs.daymeans.df$season, year))
...
London.dfg.seasonal.mean <- lapply(c("raw.cal.daymeans", "bc.b.cal.daymeans", "bc.a.cal.daymeans"), function(i){
  df <- London.dfg[[i]]
  x <- df$day
  x <- gsub(".*_", "", x)
  x <- as.numeric(x)
...
df$season_year <- ifelse(x>29&x<91, 
                         paste0(df$season, year-1), 
                         paste0(df$season, year))
...
names(London.dfg.seasonal.mean)
```
