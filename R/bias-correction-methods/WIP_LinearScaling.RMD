...
#start with tasmax
cpm.files <- cpm.files[grepl("tasmax", cpm.files)]
...
proj1 <- cpm.files[grepl("2010|2020", cpm.files)]
...
```{r}
save.image("/mnt/vmfileshare/ClimateData/Interim/WIP_LinearScaling090723.RData")
```

### Transpose and align obs and prj grids 

```{r}
tasmax.obs <- as.data.frame(obs.dfs)
row.names(tasmax.obs) <- paste0(tasmax.obs$HADsNI1980_2010_tasmax.2023.06.27.csv.x,"_", tasmax.obs$HADsNI1980_2010_tasmax.2023.06.27.csv.y)

tasmax.cal1 <- as.data.frame(cal.dfs$CPM_NI1980_1999tasmax_Run06.2023.06.27.csv)
row.names(tasmax.cal1) <- paste0(tasmax.cal1$x, "_", tasmax.cal1$y)
tasmax.cal1 <- subset(tasmax.cal1, row.names(tasmax.cal1)%in%row.names(tasmax.obs))



#Convert obs to 360 day year
