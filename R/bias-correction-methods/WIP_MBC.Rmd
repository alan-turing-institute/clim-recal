...
read_crop_df <- function(runs, #Character vector of selected runs
    ...
    if(crop == T){
      ...
      bbox <- crop.area
       ...
       name <- assign(paste0(name1, "_", cropname, year1, "_", year2, v, "_Run", i), df)
      ...
      assign(paste0(name1, "_", year1, "_", year2, v, "_Run", i), df)
...
#save.image("~/Desktop/clim-recal/clim-recal/data/MBCdata/MBCimage.1.RData") #Last saved 22.06.24
#load("~/Desktop/clim-recal/clim-recal/data/MBCdata/MBCimage.1.RData")
...
for(i in run){
  for(v in var){
    p <- paste0(dd, "Reprojected/UKCP2.2/", v, "/", i, "/latest/")
    files <- list.files(p)
    files <- files[!grepl("aux.xml", files)]
  
    files.h <- files[grepl(hist.years, files)]# Historical timeslice for calibration
    files.h.p <- paste0(p, files.h)
  ...
  # Load and convert remaining to single col dfs 
        dfL <- lapply(2:length(files.h.p), function(i){
                  p <- files.h.p[[i]] 
                  r <- rast(p)
                  r_c <- crop(r, NI.bbox) 
                  rdf <- as.data.frame(r_c) 
      ...
      assign(paste0("CPM_1980_2000", v, "_Run", i), df)
...
Using 2010-2020 as test period so just loading in 2010 here 

```{r CPM historical 2}

run <- c("05", "07", "08", "06")
var <- c("tasmax", "tasmin","pr")
hist.years <- paste0(2000:2010, "1201", collapse="|") #Historical years 

# Historical 
for(i in run){
  for(v in var){
    p <- paste0(dd, "Reprojected_infill/UKCP2.2/", v, "/", i, "/latest/")
    files <- list.files(p)
    files <- files[!grepl("aux.xml", files)]
  
    files.h <- files[grepl(hist.years, files)]# Historical timeslice 2 for calibration
    files.h.p <- paste0(p, files.h)
  ...
  # Load and convert remaining to single col dfs 
      ...
      assign(paste0("CPM_2000_2010", v, "_Run", i), df)
...
save.image("~/Desktop/clim-recal/clim-recal/data/MBCdata/MBCimage.2.RData") 
#load("~/Desktop/clim-recal/clim-recal/data/MBCdata/MBCimage.2.RData")

```

#HERE
Load later and infill CPM data -- after 2022 for both 
