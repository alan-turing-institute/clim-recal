...
save.image("~/Desktop/clim-recal/clim-recal/data/MBCdata/MBCimage.1") 
...
Cropping all to NI as above to start with 


# HERE
Sort out a better looping system so you get list nested on run or on the variable 
...
p <- paste0(dd, "Reprojected/UKCP2.2/", rep(v, 4), "/", rep(run, each=3), "/latest/")
names(p) <- run
files <- lapply(p, list.files)
names(files) <- run
...
cropped.rasts <- lapply(run, function(run){
